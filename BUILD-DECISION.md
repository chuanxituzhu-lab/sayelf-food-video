...
版本：1.7.0 · 日期：2026-09-06 · 记录在实现之前；本次为对 v1.6.0 的增量升级。
...
- **可测差异**：单文件 file:// 运行；运行依赖与自动网络请求均为 0；普通用户只填菜品和场景即可生成，也可直接输入新菜名；专业设置渐进展开；真人秀标题与人设按菜品自动切换；专业用户可录入品牌、创意、场景补充并在本机预览参照照片；分镜卡片的 Prompt 默认折叠，点击后可单独复制；支持自动匹配、9:16、3:4、16:9、4:5；四要素、菜品与真人秀证据共同编译；缺失冲突证据不产生冲突事实；已收录菜品使用专属工艺，自定义菜名使用川味通用四段工艺并标注建议；WebUI 采用截图对齐的深色工作台、顶部状态与配置入口、输入/结果双栏、横向分镜卡片、桌面吸附输入区和移动端双列示范；6–60 秒连续时间轴，首尾与用户时长精确一致；11 种菜品示范。
...
- **演进/回滚**：v1.7.0 固定规则；基于真实反馈提出变化，先完成桌面/390px 移动视觉检查与全回归后升级；保留旧 HTML 与 JSON，替换文件即可回滚。不自动更新；后续 schema 不兼容时明确拒绝导入。
