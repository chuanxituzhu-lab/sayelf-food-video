...
版本：1.7.5 · 日期：2026-09-07 · 记录在实现之前；本次为对 v1.7.4 的增量升级。
...
- **演进/回滚**：v1.7.5 固定规则；基于真实反馈提出变化，先完成桌面/390px 移动视觉检查与全回归后升级；保留旧 HTML 与 JSON，替换文件即可回滚。不自动更新；后续 schema 不兼容时明确拒绝导入。
