...
版本：1.14.0 · 日期：2026-09-19 · 记录在实现之前；本次为对 v1.13.0 的增量升级。
...
- **演进/回滚**：v1.14.0 在 v1.13.0 的事件先行、共享动效入口和 FDE 交付卡边界上增加软节奏参考；基于真实反馈提出变化，先完成桌面/390px 移动视觉检查与全回归后升级；保留旧 HTML、Skill 和 JSON，替换文件即可回滚。不自动更新；后续 schema 不兼容时明确拒绝导入。
